...
/****** Object:  StoredProcedure [dbo].[sp_UpdateStats2]    Script Date: 11/09/2026 10:26:10 ******/
        ...
        SELECT
            ...
            , PartitionsToUpdate =
                ...
                STUFF((
                    ...
                    SELECT N'','' + CONVERT(nvarchar(12), p.partition_number)

                    FROM sys.partitions p

                    LEFT JOIN sys.dm_db_incremental_stats_properties(s.object_id, s.stats_id) isp2

                        ON isp2.partition_number = p.partition_number

                    WHERE p.object_id = s.object_id

                      AND p.index_id IN (0,1) -- partitioning is aligned across indexes; stats_id is not always a valid index_id (e.g. column stats)
                      ...
                      AND
                        ...
                        (
                            ...
                            -- Partition never sampled (no incremental stats yet): force initial build, like IndexOptimize''s ModificationCounter IS NULL handling

                            (isp2.modification_counter IS NULL AND ISNULL(p.[rows],0) > 0)
...
WHERE
    ...
    (
        ...
        -- Statistics never sampled (ModificationCounter unknown): force initial build when the table has rows,

        -- matching IndexOptimize''s handling of @CurrentModificationCounter IS NULL for index/column statistics

        OR ((IndexType IN (1,2) OR IndexName IS NULL) AND ModificationCounter IS NULL AND ISNULL(CountRows,0) > 0)

        -- Incremental partition-level resample: let PartitionsToUpdate (evaluated per partition) decide

        OR (IsIncremental = 1 AND @PartitionLevel = N''Y'' AND @StatisticsResample = N''Y'' AND NULLIF(PartitionsToUpdate, N'''') IS NOT NULL)
