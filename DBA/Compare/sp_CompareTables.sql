...
BEGIN
    ...
    DECLARE @samplekeys VARCHAR(max);
    ...
    EXEC sp_executesql @sql, N'@key_diff INT OUTPUT, @samplekeys VARCHAR(max) OUTPUT', @key_diff=@key_diff OUTPUT,@samplekeys=@samplekeys OUTPUT;
    ...
    BEGIN
        ...
        SET @col_diff = 0;
        EXEC sp_executesql @sql, N'@d INT OUTPUT, @dd INT OUTPUT, @samplekeys VARCHAR(max) OUTPUT', @d=@col_diff OUTPUT, @dd=@col_distinct OUTPUT,@samplekeys=@samplekeys OUTPUT;
...
DISPLAYRESULTS:
    ...
    BEGIN
        ...
        -----------------------------------------------------------------------
        -- Buid an order by clause for easier comparison (keys then column)
        -----------------------------------------------------------------------
        ...
        --------------------------------------------------------------------------
        -- Generate a query in the select clause (vsqlquery column) for each 
        -- column with diffcount>0
        -- Each query will search in the source and in the target using the where
        -- clause store in the T_COMPARE_RESULTS.samplekeysetwhere
        -- store the result in a temp table : #GetDiffQuery
        --------------------------------------------------------------------------
        SELECT 
        'SELECT ' +@CRLF
        ...
        -------------------------------------------------------------------
        -- iterate on the #GetDiffQuery, get the vsqlquery and run it
        -------------------------------------------------------------------
