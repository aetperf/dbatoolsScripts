...
Write-Log -Level INFO -Message "STEP : ENSURING SCHEMA EXISTS IN TARGET"

$ErrorCodeEnsureSchema = 0

$startStep = Get-Date

 

$schemaExistsTarget = Invoke-DbaQuery -SqlInstance $SqlInstance -Database $TargetDB -Query @"
...
# Si aucune ligne => schéma absent

if (($schemaExistsTarget | Measure-Object).Count -eq 0) {

 

    $createSchemaQuery = "CREATE SCHEMA [$SchemaName] AUTHORIZATION [dbo];"

    $createSchemaQueryLog = $createSchemaQuery.Replace("'", "''")

 

    Write-Log -Level INFO -Message "Schema '$SchemaName' not found in target. Creating it (owner dbo)."
    ...
    if (!$WhatIf) {
        ...
        try {
            ...
            Invoke-DbaQuery -SqlInstance $SqlInstance -Database $TargetDB -Query $createSchemaQuery -EnableException
...
VALUES ($RestoreId, 'ENSURE SCHEMA IN TARGET', 'SCHEMA', '$SchemaName', '$SchemaName', 'CREATE', '$createSchemaQueryLog', 0, 'Success', GETDATE())
...
VALUES ($RestoreId, 'ENSURE SCHEMA IN TARGET', 'SCHEMA', '$SchemaName', '$SchemaName', 'CREATE', '$createSchemaQueryLog', 1, '$errorMsg', GETDATE())
...
"@
            ...
            Write-Log -Level ERROR -Message "Failed to create schema '$SchemaName' in target: $errorMsg"
        ...
        Write-Log -Level INFO -Message "[WhatIf] Would run: $createSchemaQuery"
    ...
    Write-Log -Level DEBUG -Message "Schema '$SchemaName' already exists in target."
...
$roundedDuration = [math]::Round(($endStep - $startStep).TotalSeconds, 2)

 

if ($ErrorCodeEnsureSchema -eq 1) {

    Write-Log -Level ERROR -Message "STEP : ENSURING SCHEMA EXISTS IN TARGET in $roundedDuration seconds | FAILED"

} else {

    Write-Log -Level INFO -Message "STEP : ENSURING SCHEMA EXISTS IN TARGET in $roundedDuration seconds | SUCCEED"
