...
<#
    ...
    Evolutions:
    2023-01-12 - Romain Ferraton - Add Parallel Capacities (Powershell 7+ mandatory)
...
# .\FileGroupsMoveV2.ps1 -server ".\DBA01" -dbName "AdventureWorks2019" -doWork $FALSE -onlineOpt $FALSE -tablesToMove "*" -schemaToMove "*" -TargetfileGroup "SECONDARY" -SourcefileGroup "*" -Degree 5
# .\FileGroupsMoveV2.ps1 -server "GRASRLYODTW1" -dbName "DWHD" -doWork $FALSE -onlineOpt $FALSE -tablesToMove "*" -schemaToMove "*" -TargetfileGroup "DATA" -SourcefileGroup "*" -Degree 5
param 
(
...
# A concurrent dictionnary to manage log from parallel threads
...
$duration=(Measure-Command{
    ...
    $Tables  | ForEach-Object -Parallel{
        ...
        $curtable=$_.table_name
        $curschema=$_.schema_name

        $iResultTablesIndex=$using:ResultTablesIndex
        #get a list of all non clustered indexes on this table
        $indexesNonClusteredToMove = $iResultTablesIndex.Where({$_.schema_name -eq $curschema -And $_.table_name -eq $curtable -And $_.type_desc -like "NONCLUSTERED*"}) 
        #get other idex (cluster or heap)
        $DataToMove = $iResultTablesIndex.Where({$_.schema_name -eq $curschema -And $_.table_name -eq $curtable -And $_.type_desc -notlike "NONCLUSTERED%"})


        #disable of nonclustered secondary indexes
        foreach ( $nonclusterindex in $indexesNonClusteredToMove )
        {
            $action='DISABLE SECONDARY INDEX'
            $curobject=$nonclusterindex.schema_name + "." + $nonclusterindex.table_name + "." + $nonclusterindex.index_name
            ...
            if ( $using:doWork -eq $TRUE ) {
                try {
                    $result = Invoke-DbaQuery -SqlInstance $using:server -Database $using:dbName -Query $disableindexquery
                    $silentres = $ResultsDict.TryAdd($action +" : "+ $curobject,$result)
                }
                catch {
                    Write-Host ('Failed moving index ' + $object +' to Filegroup ' + $using:TargetfileGroup + ' ' + $error[0].Exception.InnerException )
                    $silentres = $ResultsDict.TryAdd($action +" : "+ $curobject,$result)
                    #return
                }
        ...
        foreach ( $object in $DataToMove )
        {
            ...
            {
                $curobject=$object.schema_name + "." + $object.table_name
                $createtempindexquery = "CREATE CLUSTERED INDEX TempIndex ON " + $object.schema_name + "." + $object.table_name + "(" + $object.first_columnname +") ON [" + $using:TargetfileGroup+"]"
                ...
                if ( $using:doWork -eq $TRUE ) {
                    try {  
                        $action='CREATE CLUSTER INDEX ON TARGET FILEGROUP  ['+$using:TargetfileGroup+'] FOR HEAP'      
                        $result = Invoke-DbaQuery -SqlInstance $using:server -Database $using:dbName -Query $createtempindexquery
                        $silentres = $ResultsDict.TryAdd($action +" : "+ $curobject,$result)
                        $action='DROP CLUSTER INDEX ON TARGET FILEGROUP  ['+$using:TargetfileGroup+'] FOR HEAP'
                        $result = Invoke-DbaQuery -SqlInstance $using:server -Database $using:dbName -Query $droptempindexquery
                        $silentres = $ResultsDict.TryAdd($action +" : "+ $curobject,$result)
                    }
                    catch {
                        Write-Host ('Failed moving index ' + $curobject + ' to Filegroup ' + $using:TargetfileGroup + ' ' + $error[0].Exception.InnerException )
                        $silentres = $ResultsDict.TryAdd($action +" : "+ $curobject,$result)
                        #return
                    }
                }
                ...
                $curobject=$object.schema_name + "." + $object.table_name + '.' + $object.index_name
                $action='CREATE CLUSTER INDEX ON TARGET FILEGROUP ['+$using:TargetfileGroup+'] WITH DROP_EXISTING=ON' 
                $RecreateQuery=$object.CreateIndexScript.Replace('__TartgetFileGroup__',$using:TargetfileGroup)
                Write-Host($RecreateQuery)
                if ( $using:doWork -eq $TRUE ) {
                    try {        
                        $result = Invoke-DbaQuery -SqlInstance $using:server -Database $using:dbName -Query $RecreateQuery
                        $silentres = $ResultsDict.TryAdd($action +" : "+ $curobject,$result)
                    }
                    catch {
                        Write-Host ('Failed moving index ' + $object.schema_name + '.' + $object.table_name + '.' + $object.index_name + ' to Filegroup ' + $using:TargetfileGroup + ' ' + $error[0].Exception.InnerException )
                        $silentres = $ResultsDict.TryAdd($action +" : "+ $curobject,$result)
                        #return
                    }
                }
        ...
        foreach ( $nonclusterindex in $indexesNonClusteredToMove )
        {
            $action='REBUILD SECONDARY INDEX ON TARGET FILEGROUP'
            $curobject=$nonclusterindex.schema_name + "." + $nonclusterindex.table_name + "." + $nonclusterindex.index_name
            $RecreateQuery=$nonclusterindex.CreateIndexScript.Replace('__TartgetFileGroup__',$using:TargetfileGroup)
                Write-Host($RecreateQuery)
                if ( $using:doWork -eq $TRUE ) {
                    try {        
                        Invoke-DbaQuery -SqlInstance $using:server -Database $using:dbName -Query $RecreateQuery
                        $silentres = $ResultsDict.TryAdd($action +" : "+ $curobject,$result)
                    }
                    catch {
                        Write-Host ('Failed moving index ' + $curobject + ' to Filegroup ' + $using:TargetfileGroup + ' ' + $error[0].Exception.InnerException )
