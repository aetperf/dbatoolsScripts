...
$SqlInstancesToChecks | ForEach-Object -ThrottleLimit 8 -Parallel {
    ...
    try {
        $result = Test-DbaConnection -SqlInstance $SqlInstance -SkipPSRemoting -ErrorAction Continue -WarningVariable "WarningVar"
        ...
        if (!$result.ConnectSuccess) {
            Write-Host -ForegroundColor Red "$SqlInstance : KO - IP=$IpAddress | Message : $WarningVar"
        }
        else {
            Write-Host -ForegroundColor Green "$SqlInstance : OK - IP=$IpAddress | TCPPort=$TcpPort"
        }
        ...
        Write-Host -ForegroundColor Red "$SqlInstance : KO - $WarningVar - $ErrorMessage"
        continue
    }
} -ArgumentList $_.SqlInstance
